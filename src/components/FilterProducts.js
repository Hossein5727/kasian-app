    ...
    >
      ...
      {categoryList &&
        ...
        categoryList.map((item) => (
          <button
            key={item.id}
            className="flex  items-center gap-1 flex-row-reverse px-8 py-2 rounded-md cursor-pointer text-sm transition-all duration-150 hover:bg-[#212432] focus:bg-primary-color focus:text-bg-home "
          >
            {item.title}
          </button>
        ))}
